...
(deftest view-reflects-two-transactions
  (with-open [mv (view/->view *conn* '{:find [?name]
                                    :where [[?e :name ?name]]})]
...
(deftest terminal-query-error-allows-materialized-view-to-close
  ...
  (with-open [mv (view/->view *conn* '{:find [(sum ?value)]
                                    :where [(or [?e :age ?value]
                                                [?e :name ?value])]})]
    (let [key (tx-key (api/transact *conn* [{:name "Alice"}]))]
      (is (thrown-with-msg? Exception #"sum: cannot aggregate non-numeric value"
                           (view/await-tx mv key 5000)))
      (is (view/done? mv)))))
