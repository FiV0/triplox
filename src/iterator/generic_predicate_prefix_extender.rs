...
mod tests {
    ...
    fn test_binary_lt_one_var_one_const() {
        ...
        let ext = GenericPredicatePrefixExtender::new(expr, vec![], Variable::from_valid_name("?age"), 0);
    ...
    fn test_binary_lt_two_vars() {
        ...
        let expr = Expr::BinaryExpr(BinaryExpr {
            left: Box::new(Expr::Variable(Variable::from_valid_name("?a"))),
            op: BinaryOp::Lt,
            right: Box::new(Expr::Variable(Variable::from_valid_name("?b"))),
        });
        let ext = GenericPredicatePrefixExtender::new(
            expr,
            vec![(Variable::from_valid_name("?a"), 0)],
            Variable::from_valid_name("?b"),
            1,
        );
    ...
    fn test_binary_two_vars_reversed_arg_order() {
        ...
        let expr = Expr::BinaryExpr(BinaryExpr {
            left: Box::new(Expr::Variable(Variable::from_valid_name("?b"))),
            op: BinaryOp::Lt,
            right: Box::new(Expr::Variable(Variable::from_valid_name("?a"))),
        });
        let ext = GenericPredicatePrefixExtender::new(
            expr,
            vec![(Variable::from_valid_name("?a"), 0)],
