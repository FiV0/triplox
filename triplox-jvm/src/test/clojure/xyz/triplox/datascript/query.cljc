...
;; TODO We don't support collecions as
#_
(deftest test-q-coll
  (let [db [[1 :name "Ivan"]
            [1 :age  19]
...
(deftest test-bindings
  ...
  (testing "Collection binding"
    (is (= (q '[:find ?e ?name
                ...
                :where [?e :name ?name]]
              ["Ivan"])
           #{[8796093022208 "Ivan"] [8796093022210 "Ivan"]})))
...
;; TODO consider if we want to support symbol data types
#_
...
(deftest test-constant-substitution
  ...
  (let [cnt+q (fn [query db & sources]
                (set (apply d/q db query sources)))
        db (d/db *conn*)]
    (is (= #{["5b"]} (cnt+q '[:find ?v :where [5 :b ?v]] db)))
    #_(is (= [1 #{[:b]}]   (cnt+q '[:find ?a :where [5 ?a "5b"]] db)))
    (is (= #{[5]}    (cnt+q '[:find ?e :where [?e :b "5b"]] db)))
    #_(is (= [1 #{[5 :b "5b"]}] (cnt+q '[:find ?e ?a ?v :in ?e ?a :where [?e ?a ?v]] db 5 :b)))
    #_(is (= [2 #{[5 :b "5b"]}] (cnt+q '[:find ?e2 ?a ?v :in ?a ?v :where [?e ?a ?v] [?e2 ?a ?v]] db :b "5b")))
    #_(is (= [3 #{[:a "5a"] [:b "5b"] [:c "5c"]}] (cnt+q '[:find ?a ?v :in ?e :where [?e ?a ?v]] db 5)))
    #_(is (= [1 #{[5 :b]}] (cnt+q '[:find ?e ?a :where [?e ?a "5b"]] db)))
    #_(is (= [1 #{[5 :b]}] (cnt+q '[:find ?e ?a :in ?v :where [?e ?a ?v]] db "5b")))
    #_(is (= [1 #{[5 :b]}] (cnt+q '[:find ?e ?a :in [?v ...] :where [?e ?a ?v]] db ["5b"])))
    #_(is (= [1 #{[5 :b]}] (cnt+q '[:find ?e ?a :where [(ground "5b") ?v] [?e ?a ?v]] db)))))
