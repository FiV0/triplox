...
mod tests {
    ...
    #[test]
    fn placeholder_only_not_has_an_empty_internal_interface() {
        let query = parse_query("[:find ?e :where [?e :name _] (not [_ :age _])]").unwrap();
        let rewritten = rewrite_query(&query);
        let WhereClause::NotJoin(not) = &rewritten.where_clauses[1] else {
            panic!("expected NOT");
        };
        assert_eq!(not.unify_vars, UnifyVars::Explicit(BTreeSet::new()));
    }
